...
def updateManyDocument(collectionName, updateQueries):
    bulkOps = []
    for query in updateQueries:
        bulkOps.append(UpdateOne({'_id': query['_id']}, {'$set': query}))
    try:
        inserted = connectedDB[collectionName].bulk_write(bulkOps)
        return inserted.bulk_api_result
    except Exception as err:
        print('Could not update into the collection ', collectionName)
        print('Because of ', err)
        return []
